...
for f in files:
    ...
    if split2[1] == 'Np'+N_planetesimals:
        try:
            ff = open(f, 'r')
            lines = ff.readlines()
            length = len(lines)
...
axes[0].plot(time, Eavg, '.', markeredgecolor='none', color='darkgreen', label='PLANETESIMAL avg.')
axes[0].plot(time,0.6e-11*time**(0.5),color='black', label='t$^{ 0.5}$')
axes[0].plot(time,1e-13*time,color='red', label='t')
...
if swifter == 1:
    ...
    for i in xrange(0,N_files):
        ...
        if j>=lc:
            j=0
    axes[0].plot(time, Eavg, '.', markeredgecolor='none', color='darkblue', label='Swifter Avg.')
