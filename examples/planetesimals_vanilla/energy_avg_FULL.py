...
for i in xrange(0,n_it):
    for j in range(0,N_files):
        ...
        A[j][i] = vals_for_med[j]
    Aavg[i] = np.median(vals_for_med)
    ...
    if time_sort == 1:
        if i > 0:
            if names[i] != names[i-1]:
                plt.plot(time,A[i], ms=ms, color=colors[i], alpha=alpha, label = names[i])
            else:
                plt.plot(time,A[i], ms=ms, color=colors[i], alpha=alpha)
        else:
            plt.plot(time,A[i], ms=ms, color=colors[i], alpha=alpha, label = names[i])
    else:
        plt.plot(time,A[i], ms=ms, color=colors(i), alpha=alpha, label = names[i])
...
if plot_choice == 1:
    ...
    print 'max error is',max(A[i])
else:
    dt = time[1:] - time[0:-1]
    a = Aavg[1:] - 0.1*dt
    plt.plot(time[1:],a,'ko')
