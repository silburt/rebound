...
if diagnostics == 1:
    ...
    axes[0].plot(data[:,0],1e-11*data[:,0],color='red',label='t growth')
    axes[0].plot(data[:,0],1e-11*data[:,0]**0.5,color='black',label='t^1/2 growth')
    axes[0].legend(loc='upper left',prop={'size':10})
    axes[0].set_xscale('log')
    axes[0].set_yscale('log')
    #axes[0].set_xlim([20,30])
    axes[0].set_xlim([1e-3,max(data[:,0])])
    ...
    axes[2].plot(data[:,0],data[:,4], 'o', ms=ms, markeredgecolor='none',label='elapsed time (s)')
    axes[2].plot(data[:,0],data[:,5], 'o', ms=ms, markeredgecolor='none',label='total # CE')
