/**
 * A.S. This simulates the massive outer SS.
 *
 *
 */
#include <stdio.h>
...
int main(int argc, char* argv[]){
    ...
    r->dt = 0.0312*6.2832;                          //yr/2*pi (planet @ 1AU takes 2*pi years to orbit)
    ...
    if(mercury_swifter_comp){
        mercury_dir = argv[5];
        swifter_dir = argv[6];
        output_to_mercury_swifter(r, HSF, tmax, n_output);
    }
...
//    fprintf(mercuryparams," output interval (days) = %.2fd0\n",(tmax/n_output)*yrtwopi2day);
    ...
    fprintf(mercuryparams," < not used at present >\n");
    fprintf(mercuryparams," < not used at present >\n");
    fprintf(mercuryparams," Hybrid integrator changeover (Hill radii) = %f\n",HSR);
