...
if diagnostics == 1:
    ...
    axes[0].plot(time,1e-9*time, color='red', label='t')
    axes[0].plot(time,1e-9*time**0.5, color='black', label='t^0.5')
    ...
    axes[1].set_xscale('log')
    axes[2].plot(time,data[:,6], 'o', ms=ms, markeredgecolor='none',label='HSF')
    axes[2].plot(time,data[:,7], 'o', ms=ms, markeredgecolor='none',label='SSF')
    axes[2].set_ylabel('HSF')
